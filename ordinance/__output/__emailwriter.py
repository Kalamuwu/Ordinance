...
class EmailWriter(WriterBase):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        ############################################
        super().__init__(config, config.get_config('writers.email.frequency_sec'))
        self.server = {
            'ip': config.get_config('writers.email.smtp.ip'),
            'port': config.get_config('writers.email.smtp.port', 25)
        }
        self.credentials = {
            'username': config.get_config('writers.email.credentials.username'),
            'password': config.get_config('writers.email.credentials.password'),
        }
        self.meta = {
            'from': config.get_config('writers.email.meta.from', "Ordinance_Incident@localhost"),
            'to': config.get_config('writers.email.meta.to'),
            'title': config.get_config('writers.email.meta.title', "Ordinance Alerts")
        }
